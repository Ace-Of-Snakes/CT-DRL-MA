...
def _patch_truck_factory_signatures(factory):
    """
    Monkey-patch TruckFactory methods to accept (and ignore) the extra
    parking_spot_prefix kwarg that generate_trucks() passes but the
    underlying methods do not declare.
    """
    orig_create_delivery = factory._create_delivery_truck.__func__
    orig_generate_pickup = factory._generate_pickup_trucks.__func__

    @functools.wraps(orig_create_delivery)
    def patched_create_delivery(self, *, containers, day_key, base_date, parking_spot_prefix=None):
        return orig_create_delivery(self, containers=containers, day_key=day_key, base_date=base_date)

    @functools.wraps(orig_generate_pickup)
    def patched_generate_pickup(self, *, containers, day_key, base_date, parking_spot_prefix=None):
        return orig_generate_pickup(self, containers=containers, day_key=day_key, base_date=base_date)

    import types
    factory._create_delivery_truck = types.MethodType(patched_create_delivery, factory)
    factory._generate_pickup_trucks = types.MethodType(patched_generate_pickup, factory)
...
class TestTruckFactory(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Patch signature mismatches in source (generate_trucks passes
        # parking_spot_prefix to methods that don't accept it).
        _patch_truck_factory_signatures(self.factory)
    ...
    def _create_test_containers(self, n=10):
        ...
        for i in range(n):
            container = Container(
                ...
                direction=Direction.IMPORT if i % 2 == 0 else Direction.EXPORT,
                container_type="FEU",
    ...
    def test_truck_length_fixed(self):
        ...
        for truck in trucks:
            self.assertEqual(truck.max_length, STANDARD_VEHICLE_LENGTH_M)
    ...
    def test_delivery_truck_generation(self):
        ...
        for truck in trucks:
            ...
            self.assertLessEqual(total_length, STANDARD_VEHICLE_LENGTH_M)
    ...
    def test_parking_spot_assignment(self):
        """Test that parking spots are None (agent handles assignment at runtime)."""
        trucks = self.factory.generate_trucks(
            day_of_week="Monday",
            delivery_containers=self.test_containers[:3],
        ...
        # Parking spot assignment is deferred to the RL agent,
        # so trucks are created with parking_spot=None.
        for truck in trucks:
