...
class TestWagon(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        self.assertEqual(wagon1.length, STANDARD_VEHICLE_LENGTH_M)
        self.assertEqual(len(wagon1.containers), 0)
        self.assertEqual(len(wagon1.pickup_container_ids), 0)

        # Custom length
        wagon2 = Wagon("W002", length=STANDARD_VEHICLE_LENGTH_M)
        self.assertEqual(wagon2.length, STANDARD_VEHICLE_LENGTH_M)
    ...
    def test_get_available_length(self):
        ...
        # Empty wagon
        self.assertEqual(self.wagon.get_available_length(), STANDARD_VEHICLE_LENGTH_M)

        # Add container
        self.wagon.add_container(self.small_container)
        expected = STANDARD_VEHICLE_LENGTH_M - 6.06
        self.assertAlmostEqual(self.wagon.get_available_length(), expected, places=2)

        # Add another container
        self.wagon.add_container(self.medium_container)
        expected = STANDARD_VEHICLE_LENGTH_M - 6.06 - 12.19
        self.assertAlmostEqual(self.wagon.get_available_length(), expected, places=2)
    ...
    def test_is_full(self):
        ...
        # Should be full if less than MIN_CONTAINER_LENGTH_M available
        available = self.wagon.get_available_length()
        if available < MIN_CONTAINER_LENGTH_M:
    ...
    def test_edge_cases(self):
        ...
        exact_container.length = STANDARD_VEHICLE_LENGTH_M
