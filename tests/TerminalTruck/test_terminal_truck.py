...
class TestTerminalTruckInitialization(unittest.TestCase):
    ...
    def test_default_initialization(self):
        ...
        self.assertTrue(truck.truck_id.startswith("TTR"))
        self.assertEqual(truck.max_length, STANDARD_VEHICLE_LENGTH_M)
        self.assertEqual(truck.status, TerminalTruckStatus.IDLE)
...
class TestContainerManagement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create mock containers with is_swap_body/is_trailer attributes
        ...
        self.trailer.is_swap_body = False
        self.trailer.is_trailer = True
        ...
        self.swap_body.is_swap_body = True
        self.swap_body.is_trailer = False
        ...
        self.regular_container.is_swap_body = False
        self.regular_container.is_trailer = False
    ...
    def test_add_none_container(self):
        """Test that None container is rejected (returns False)."""
        result = self.truck.add_container(None)
...
class TestTaskManagement(unittest.TestCase):
    ...
    def test_assign_task_normal(self):
        ...
        self.assertEqual(self.truck.status, TerminalTruckStatus.BUSY)
    ...
    def test_assign_task_no_time_uses_default(self):
        """Test that not providing task_time uses the default duration."""
        self.truck.assign_task(
            source="YARD_A",
            destination="YARD_B",
            task_time=None,  # Should default to TERMINAL_TRUCK_TASK_DURATION_S
            current_time=self.base_time
        )

        expected_completion = self.base_time + timedelta(seconds=TERMINAL_TRUCK_TASK_DURATION_S)
        self.assertEqual(self.truck.task_completion_time, expected_completion)
    ...
    def test_complete_task(self):
        ...
        container.is_swap_body = False
        container.is_trailer = True
        ...
        self.assertEqual(self.truck.status, TerminalTruckStatus.IDLE)
...
class TestStringRepresentations(unittest.TestCase):
    ...
    def test_repr(self):
        ...
        truck.status = TerminalTruckStatus.BUSY

        container = Mock(spec=Container)
        container.container_type = "Swap Body"
        container.container_id = "SWAP_001"
        container.length = 7.45
        container.is_swap_body = True
        container.is_trailer = False
        truck.add_container(container)
