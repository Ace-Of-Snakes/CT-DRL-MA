...
class WarpTrainingLogger:
    ...
    def __init__(self, log_dir='logs', experiment_name=None, quiet_console=True):
        ...
        if experiment_name is None:
            ...
            experiment_name = f"warp_terminal_training_{timestamp}"
        ...
        # Log start message
        self.logger.info(f"Started Warp-accelerated training experiment: {experiment_name}")
...
class WarpCurriculumTrainer:
    """
    Curriculum learning trainer for container terminal agents using Warp for GPU acceleration.
    ...
    def __init__(self, 
        ...
        """Initialize the Warp-accelerated curriculum trainer."""
        ...
        # Initialize logger
        self.logger = WarpTrainingLogger(log_dir, experiment_name, quiet_console=quiet_console)
        
        # Log GPU device information
        if device == 'cuda' and torch.cuda.is_available():
            cuda_device = torch.cuda.current_device()
            self.logger.logger.info(f"Using PyTorch on GPU: {torch.cuda.get_device_name(cuda_device)}")
        ...
        self.curriculum_stages = [
            # Stage 1: Basic terminal operations (small terminal)
            {
                ...
                'num_railtracks': 2,
                'num_railslots_per_track': 5,
                'num_storage_rows': 3,
                'num_terminal_trucks': 1,
                'max_trucks_per_day': 8,
    ...
    def create_environment(self, stage_config):
        ...
        env = WarpTerminalEnvironment(
            terminal_config=None,  # No config needed for Warp version
            ...
            num_rail_tracks=stage_config['num_railtracks'],
            num_rail_slots_per_track=stage_config['num_railslots_per_track'],
            num_storage_rows=stage_config['num_storage_rows'],
            num_storage_bays=stage_config['num_railslots_per_track'] * 2,  # Match the storage-to-rail ratio
            device=self.device,
    ...
    def train(self, total_episodes=None, resume_checkpoint=None):
        ...
        self.logger.logger.info(f"Starting Warp-accelerated curriculum training with {len(self.curriculum_stages)} stages")
        stage_rewards = []
        ...
        try:
            ...
            # Print performance stats from environment
    ...
    def train_episode(self, env, agent):
        """Train for a single episode with Warp acceleration."""
        # Reset environment
        state, _ = env.reset()
        episode_reward = 0
        episode_steps = 0
        ...
        while not done and not truncated and env.current_simulation_time < max_simulation_time:
            ...
            try:
                ...
                # Use environment's create_wait_action method
                wait_action = env.create_wait_action()
