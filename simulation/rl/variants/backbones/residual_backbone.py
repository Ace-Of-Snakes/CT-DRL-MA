# simulation/rl/variants/backbones/residual_backbone.py
"""Residual CNN backbone — skip connections around spatial reasoning layers.

Tests: "Do skip connections improve gradient flow for this deep spatial encoder?"

Layer structure (same 4 layers as baseline, skip connections added):
...
Skips are identity (no projection needed — channels don't change).
Zero extra parameters over baseline.
"""
import torch
...
class ResidualCNNBackbone(nn.Module):
    """FactoredCNNBackbone with residual skip connections.

    Skip connections around conv2 and conv3 blocks:
      x = x + ReLU(GN(conv2(x)))
      x = x + ReLU(GN(conv3(x)))
    ...
    def __init__(self, cfg: CNNConfig):
        ...
        # Stage 2: (3,1,3) — cross-region/tier + SKIP
